...
func (s *Server) Remainder(ctx context.Context, request *arifmeticService.Request) (*arifmeticService.Response, error) {
	a, b := request.GetA(), request.GetB()

	result := int(a) % int(b)

	return &arifmeticService.Response{Result: float64(result)}, nil
}
